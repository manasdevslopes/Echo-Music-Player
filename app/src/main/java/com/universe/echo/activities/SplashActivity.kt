...
class SplashActivity : AppCompatActivity() {
    ...
    * which provides us with that.*/
    /*The Manifest class contains the different permissions used in Android. Let's understand what each permission is used for.*/
    var permissionsString = arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE, /*This allows the application to read from external storage.We need this to read the songs on your device*/
            Manifest.permission.MODIFY_AUDIO_SETTINGS, /*Allows an application to modify the global audio settings. This helps  us in turning the volume up or down when music is playing*/
            Manifest.permission.READ_PHONE_STATE, /*This gives the application access to get the cellular network information, phone state including the phone number, the status of any ongoing calls, and a list of any phone accounts registered on the device. This checks whether the phone is active or in flight mode.*/
            Manifest.permission.PROCESS_OUTGOING_CALLS, /*Allows the application to see the number dialed during an outgoing call and gives it an option to redirect or abort the call. Helps us in checking when a call is made so that we can pause the media player*/
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        * Now, you will be wondering what is this R file and why are we doing R.layout before mentioning the name of the activity.
        * This is done so that the compiler understands as to which file is to be displayed as the view to this activity.
        * The R file is the auto-generated file which contains the id's for each file present in our Android project.
        * The layout directory is present inside the R file and the activity_splash is present inside layout directory.
        * Hence we use R.layout.activity_splash*/
        /*Here we check if the user has granted the permissions to the activity or not. If yes, then we want to execute the else block
        * otherwise we execute the if block*/
        if (!hasPermissions(this@SplashActivity, *permissionsString)) {
            /*Now when the permissions were not granted, we want our application to ask for permissions. This is done with the help of
...
//            }, 1000)
    ...
    * result of the permissions request. This was eatlier not present till the android version 6.0.0(Marshmallow) or API 23*/
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        when (requestCode) {/*Here we check the request code of the permission request made by the user*/
...
//                    }, 1000)
                    timeForHandler()
                    /*If in case any permission was denied by the user the application exits giving a message to the user that, you haven't
                    *granted all the permission and this application needs them in order to continue*/
                } else {
                    /*The toast message is a small prompt which appears at the bottom of the screen*/
                    Toast.makeText(this@SplashActivity, "Please grant all the permissions to continue.", Toast.LENGTH_SHORT).show()
                    this.finish()
                }
                /*Here we add the return statement to prevent the further execution of the code. The control returns outside of this function
                * and now application has nowhere to go. Hence after the execution of if-else nothing happens i.e either new activity
                * launches or the applicaiton exits but does not remain running in the background thus consuming the resources*/
                return
            }
            else -> {
                /*In cases where something with the Android OS goes wrong, we want our application to exit thus telling the user that
                * somethings has gone wrong. In such cases try re-opening the application*/
    ...
    /*As the name suggests, this method checks whether the user has granted the permissions asked by the application*/
    fun hasPermissions(context: Context, vararg permissions: String): Boolean { /*This functions takes two params namely the context of the
                                                                                 application and the array of permissions. vararg is used to make an array into simple array.*/
    ...
    /*The handler is used to handle the tasks i.e. it is used to delay the tasks which need to be
      performed. Here we are delaying the opening of the next activity by 1000ms (1 sec) in order to make the welcome/splash screen visible
      to the user and give our app a nice user experience(known as the UX)*/
    fun timeForHandler(){
        Handler().postDelayed({
